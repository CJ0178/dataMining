<div id="dataset" class="text-center pt-16 pb-8">
    <p class="text-4xl font-bold">
        Data Sample Penduduk Kenya di Daerah Pedalaman pada Tahun 2015
    </p>
    <div class="flex gap-3 justify-center items-center">
        <p class="text-4xl font-bold">
            dari
        </p>
        <img src="logoKaggle.png" class="w-32 mt-3" alt="">
    </div>
</div>

<div class="flex flex-col w-[98%]">
    <div class="overflow-x-auto sm:-mx-6 lg:-mx-8">
      <div class="inline-block w-full py-2 sm:px-6 lg:px-8">
        <div class="overflow-auto">
          <table
            class="min-w-full border text-center text-sm font-light dark:border-neutral-500">
            <thead class="border-b dark:border-neutral-500">
              <tr class="bg-[#5C9998]">
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  Survey_id
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  Ville_id
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  sex
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  Age
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  Married
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  Number_children
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  education_level
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  total_members
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  gained_asset
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  durable_asset
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  save_asset
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  living_expenses
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  other_expenses
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  incoming_salary
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  incoming_own_farm
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  incoming_business
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  incoming_no_business
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  incoming_agricultural
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  farm_expenses
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  labor_primary
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  lasting_investment
                </th>
                <th
                  scope="col"
                  class="border-r p-2 text-white text-lg dark:border-neutral-500">
                  no_lasting_investmen
                </th>
                <th scope="col" class="p-2 text-white text-lg">depressed
                </th>
              </tr>
            </thead>
            <tbody>
              <tr class="border-b dark:border-neutral-500">
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  926
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  91
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  4
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  10
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  5
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28912201
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  22861940
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  23399979
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  26692283
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28203066
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  30028818
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  31363432
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28411718
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28292707
                </td>
                <td class="whitespace-nowrap px-6 py-4">0</td>
              </tr>
              <tr class="border-b dark:border-neutral-500">
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  747
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  57
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  23
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  3
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  8
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  5
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28912201
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  22861940
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  23399979
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  26692283
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  28203066
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  30028818
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  31363432
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28411718
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28292707
                </td>
                <td class="whitespace-nowrap px-6 py-4">1</td>
              </tr>
              <tr class="border-b dark:border-neutral-500">
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1190
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  115
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  22
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  3
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  9
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  5
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  28912201
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  22861940
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  23399979
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  26692283
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  28203066
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  30028818
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  31363432
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  28411718
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  28292707
                </td>
                <td class="whitespace-nowrap px-6 py-4">0</td>
              <tr class="border-b dark:border-neutral-500">
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1065
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  97
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  27
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  2
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  10
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  4
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  52667108
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  19698904
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  49647648
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  397715
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  44042267
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  1
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  22288055
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  18751329
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  7781123
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  69219765
                </td>
                <td class="whitespace-nowrap px-6 py-4">0</td>
              <tr class="border-b dark:border-neutral-500">
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  806
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  42
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  59
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  4
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  10
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  6
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  82606287
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  17352654
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  23399979
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  80877619
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  74503502
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  0
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  0
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  53384566
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  20731006
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  1
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4  dark:border-neutral-500">
                  20100562
                </td>
                <td
                  class="whitespace-nowrap text-base border-r px-6 py-4 dark:border-neutral-500">
                  43419447
                </td>
                <td class="whitespace-nowrap px-6 py-4">0</td>
              </tr>
            </tbody>
          </table>
        </div>
      </div>
    </div>
</div>

<div class="py-14 pt-24">
    <p class="text-3xl font-bold text-[#2C7877] mb-8 underline">
        Penjelasan Atribut
    </p>
</div>

<div class="flex flex-col items-center justify-around">
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Survey_id</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Pengidentifikasi unik setiap survei
            <br> <span class="font-bold text-[#2C7877] text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Kode numerik untuk area dalam pedesaan
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Ville_id</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-20 py-4 h-auto text-[#2C7877]">Sex</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Laki-laki (1) atau perempuan (0)
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Umur
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-[60px] py-4 h-auto text-[#2C7877]">Age</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Married</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Status sudah menikah (1) atau belum menikah (0)
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Jumlah anak dibawah 18 tahun yang dimiliki
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Number_children</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Education_level</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Jumlah tahun pendidikan formal yang telah diselesaikan
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Jumlah orang satu keluarga yang tinggal bersama
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-14 py-4 h-auto text-[#2C7877]">Total_members</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Gained_asset</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Akumulasi aset yang diperoleh selama periode tertentu
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Aset fisik atau kepemilikan yang tahan lama
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-[52px] py-4 h-auto text-[#2C7877]">Durable_asset</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-14 py-4 h-auto text-[#2C7877]">Save_asset</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Pendapatan atau aset yang disimpan untuk masa depan
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Biaya reguler untuk kehidupan sehari-hari
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Living_expenses</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Other_expenses</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Biaya tambahan lainnya
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Penghasilan dari pekerjaan organisasi atau perusahaan
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Incoming_salary</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-[52px] py-4 h-auto text-[#2C7877]">Incoming_own_farm</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Penghasilan dari kegiatan pertanian
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Penghasilan dari bisnis sendiri atau usaha wirausaha
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Incoming_business</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Incoming_no_business</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Penghasilan dari sumber lain, seperti investasi atau aliran pendapatan pasif
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Jumlah pendapatan dari kegiatan pertanian
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Incoming_agricultural</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-16 py-4 h-auto text-[#2C7877]">Farm_expenses</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Biaya untuk operasi pertanian
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Pekerja yang menjadi tulang punggung keluarga
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Labor_primary</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">Lasting_investment</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Jumlah investasi jangka panjang
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
    </div>
    <div class="flex items-center justify-end pr-64 my-14 w-full">
        <p class="mr-6 text-xl text-end">Jumlah investasi jangka pendek
            <br> <span class="font-bold text-[#2C7877]">(Integer)</span>
        </p>
        <hr class="border-[#96D0BD] border-2 w-12">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-10 py-4 h-auto text-[#2C7877]">No_lasting_investmen</div>
    </div>
    <div class="flex items-center pl-64 w-full">
        <div class="text-3xl bg-[#96D0BD] rounded-full w-fit px-14 py-4 h-auto text-[#2C7877]">Depressed</div>
        <hr class="border-[#96D0BD] border-2 w-12">
        <p class="pl-6 text-xl">Mengalami depresi (1) atau tidak depresi (0)
            <br> <span class="font-bold text-[#2C7877]">(Boolean)</span>
        </p>
    </div>
</div>
